...
### 注: mysql存储过程每一句后面必须用;结尾，使用的临时字段需要在定义游标之前进行声

## 创建游标 

#### 为什么要创建 "游标" 要是查看表中有2000多条数据，难道要执行2000多次？ 

#### 显然是不现实的；最终找到写一个存储过程然后通过循环来更新C表

#### 然而存储过程中的写法用的就是游标的形式：如下：

```puml
 # CREATE PROCEDURE [存储过程名](参数) BEGIN   ---调用一个存储过程名并重[BEGIN]当前开始
 # DECLARE [游标名] CURSOR FOR    ---创建一个人游标名并循环
 # SELECT [列] FROM [表] END   ---查找数据
```

## 打开，关闭 游标

### 游标不在需要时都应该关闭，减少不必要的资源浪费
